...
```
-> python3 fall_detection.py -h
usage: fall_detection.py [-h] [-e] [-i INPUT] [--pd_m PD_M] [--lm_m LM_M] [-xyz] [-c]
...
- Show only the camera visualization whit the fall detector working:
    ...
    ```python3 fall_detection.py -e```

- Additionally display the skeleton in 3D:

    ```python3 fall_detection.py -e -3 image```
